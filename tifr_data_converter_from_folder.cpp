...
#include <sstream>
#include <cstdio>
#include <sys/types.h>//for directory traversing
...
int main()
{
   DIR *dir;
	...
	dir= opendir("/home/rituparna/Desktop/MSC_Project/data_and_results/real_data/c217/DATA_ire");

   int count=0;
	while( (sd=readdir(dir)) !=NULL)
	{
		string fname=sd->d_name;
		
		if (fname!= "." && fname != "..")
		{
         string f_dir= "/home/rituparna/Desktop/MSC_Project/data_and_results/real_data/c217/DATA_ire/";
   
         string f_path=f_dir+fname;//Concatenating 2 strings
         
         char file_name[100];

         strcpy(file_name, f_path.c_str());  // converting string to char 
         //strcat(f_dir, fname);
      
         TFile *f1 = new TFile(file_name,"READ","My root file1");
   ...
   string fout_dir= "/home/rituparna/Desktop/MSC_Project/data_and_results/real_data/c217/DATA_ASCII/";//directory to store ASCII files
   string ext=".txt";
   size_t lastindex = fname.find_last_of("."); 
   string raw_fname = fname.substr(0, lastindex); //Extracting the raw filename

   string fout_path=fout_dir+raw_fname+ext;//Concatenating strings

   char fout_name[200];
   strcpy(fout_name, fout_path.c_str());  // converting string to char 

   FILE* f = fopen(fout_name, "w");  
